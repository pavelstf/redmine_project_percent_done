# Staging Runbook: Project Percent Done 1.1.1

Target: GCR Redmine staging (`https://www.redminestaging.gcr.bg`)
...
```text
/home/gcrbgcaa/redminestaging/plugins/redmine_project_percent_done-1.1.1-staging-20260710.zip
```
...
- Business logic, Public API V1, REST behavior, and calculation algorithm:
...
ZIP_FILE="redmine_project_percent_done-1.1.1-staging-20260710.zip"
...
echo "== Recent staging production.log tail =="
tail -n 120 log/production.log

echo "== Recent staging production.log error check =="
...
echo "== Done =="
echo "Staging package installed. If needed, rollback backup path is stored in:"
echo "$LAST_BACKUP_FILE"
```

## Rollback Commands

Run only if staging validation fails and the previous plugin directory must be
restored from the backup generated by the install block.
...
- `Backup: ...redmine_project_percent_done-staging-backup-<timestamp>.tar.gz`
- version checks:
  - `version ProjectPercentDone::PLUGIN_VERSION`
  - `PLUGIN_VERSION = '1.1.1'`
- `No native title tooltip in settings partial.`
- `test -f public/plugin_assets/.../project_percent_done.css` success
...
- Rails runner output:
  ...
  - `help_keys_checked=9`
  - `capabilities={...}`
- final `Recent staging production.log error check` block
